...
host name is derivable -- from the TLS ClientHello server-name extension, and
from DNS queries -- and a URL PATH IS NOT. This module never invents one. A
plaintext HTTP request line is read when there is one, and it is labelled as
such, so the page can tell a measured path from an absent one.
...
distinct flows, :data:`MAX_NAME_CHARS` per host name and
:data:`MAX_TARGET_CHARS` per request line. Every one of them is reported when
...
#: Characters of one plaintext request line quoted into the report. Long enough
#: for a real path with a query string, short enough that a request line cannot
#: become the page.
MAX_TARGET_CHARS = 200

#: Bytes of a TCP payload examined for a request line and its Host header.
#: INDEPENDENT of MAX_TARGET_CHARS on purpose: sizing the window from the cap on
#: the quoted target made a smaller cap stop the parser finding the request line
#: at all, so tightening one bound silently disabled the whole reader.
MAX_REQUEST_HEAD_BYTES = 2048

#: Request lines kept per row. A row is one host; a handful of examples shows
#: the shape of what was called without turning the row into a log.
MAX_TARGETS_PER_ROW = 8
...
def http_request(payload: bytes):
    """``{"host", "target"}`` for a plaintext HTTP request line, or None."""
    ...
    for raw in head.split(b"\r\n")[1:33]:
        if raw[:5].lower() == b"host:":
            ...
            break
    try:
        target = (parts[0] + b" " + parts[1]).decode("ascii", errors="strict")
    except UnicodeDecodeError:
        return None
    return {"host": host, "target": target[:MAX_TARGET_CHARS]}
...
def _client_evidence(layer: dict):
    ...
    if request:
        return {"kind": "http", "host": request["host"], "target": request["target"]}
    if layer["syn"]:
        return {"kind": "syn"}
    return None
...
def _absorb(conv: dict, layer: dict) -> None:
    ...
    if kind == "sni":
        ...
        if target not in conv["targets"] and len(conv["targets"]) < MAX_TARGETS_PER_ROW:
            conv["targets"].append(target)
...
def _merge(rows: dict, row: dict) -> None:
    ...
    for target in row["targets"]:
        if target not in held["targets"] and len(held["targets"]) < MAX_TARGETS_PER_ROW:
...
def _rows(flows: dict) -> list:
    ...
    for conv in flows.values():
        ...
        if conv["sni"]:
            ...
            row.update(
                {
                    "host": conv["http_host"] or server[0],
                    "port": server[1],
                    "proto": PROTO_HTTP,
                    "source": SOURCE_HTTP,
